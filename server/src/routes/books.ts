...
books.post("/", protectedRoute, adminRoute, async (req: AuthRequest, res) => {
  ...
  if (!book || Object.keys(books).length === 0) {
    return res
      .status(400)
      .send({ message: bookController.errors.DETAILS_MISSING });
  }
  ...
  try {
    ...
    return res
      .status(201)
      .send({ result, message: bookController.success.CREATED });
  } catch (error) {
...
books.put(
  ...
  async (req: AuthRequest, res) => {
    ...
    try {
      ...
      return res
        .status(200)
        .send({ result, message: bookController.success.UPDATED });
    } catch (error) {
      return res.status(500).send({ message: error });
    }
  }
...
books.delete(
  ...
  async (req: AuthRequest, res) => {
    ...
    try {
      ...
      return res
        .status(202)
        .send({ result, message: bookController.success.DELETED });
    } catch (error) {
      if (error === bookController.errors.NOT_FOUND)
