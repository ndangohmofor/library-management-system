...
class ReviewsController {
  errors = {
    ...
    DETIALS_MISSING: "Review details are missing",
    REVIEW_ID_MISSING: "Review id is missing",
    NOT_FOUND: "Review is not found",
  };
  ...
  public async createReview(
    bookId: string,
    reviewBody: { text: string; rating: number },
    userName: string
  ): Promise<{ insertResult: InsertOneResult; updateResult: UpdateResult }> {
    const review = {
      _id: null,
    ...
    const updateResult = await collections?.reviews?.updateOne(
      { _id: bookId },
      {
        $push: {
          reviews: {
